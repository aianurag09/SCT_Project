...
def main():
    ...
    #data = loadtxt('Output_PCA.csv',delimiter='\n')
    #data1 = loadtxt('Dates.csv',delimiter='\n')

    f = open('Output_PCA.csv','r')

    f1 = open('Industries.txt','r')

    for line in f1:
        Industries = line.split(',')
    f1.close()

    List = random.sample(range(147), 147)   
    ...
    for i, c in enumerate(clusters):
        for p in c.points:
            print " Cluster: ", i, "\t Point :", p ,"\t Industry :" , Industries[List[j]]
            j = j + 1
